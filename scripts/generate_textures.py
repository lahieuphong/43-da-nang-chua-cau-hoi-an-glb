...
def texture_weathered_stone() -> None:
    """Đá vôi/lime-wash sáng hơn, giống lan-can và chân cầu trong ảnh tham chiếu."""
    rng = random.Random(201)
    img = noise_base((205, 200, 182), 15, 202)
    draw = ImageDraw.Draw(img, "RGBA")
    height = Image.new("L", (SIZE, SIZE), 150)
    hd = ImageDraw.Draw(height)

    # Mảng loang khô vàng-xám, tránh xanh xám đậm của bản trước.
    for _ in range(120):
        x, y = rng.randrange(SIZE), rng.randrange(SIZE)
        r = rng.randint(8, 44)
        fill = (rng.randint(170, 218), rng.randint(166, 205), rng.randint(145, 182), rng.randint(14, 45))
        draw.ellipse([x - r, y - r // 2, x + r, y + r // 2], fill=fill)
    # Vết nứt và ron đá mảnh.
    for _ in range(82):
        x0, y0 = rng.randrange(SIZE), rng.randrange(SIZE)
        pts = [(x0, y0)]
        for _ in range(rng.randint(2, 5)):
            x0 += rng.randint(-34, 34)
            y0 += rng.randint(10, 46)
            pts.append((x0, y0))
        draw.line(pts, fill=(92, 88, 78, rng.randint(28, 72)), width=1)
        hd.line(pts, fill=95, width=1)
    # Chỉ đá ngang/đứng rất nhẹ để phần đế giống khối đá thật.
    for y in range(24, SIZE, 84):
        draw.line([(0, y), (SIZE, y + rng.randint(-3, 3))], fill=(140, 135, 119, 55), width=1)
        hd.line([(0, y), (SIZE, y)], fill=118, width=1)
    for x in range(10, SIZE, 112):
        draw.line([(x, 0), (x + rng.randint(-3, 3), SIZE)], fill=(145, 139, 122, 38), width=1)
        hd.line([(x, 0), (x, SIZE)], fill=122, width=1)

    img = img.filter(ImageFilter.GaussianBlur(0.25))
    save(img, "weathered_stone_basecolor.png")
    save(normal_from_height(height.filter(ImageFilter.GaussianBlur(0.85)), 2.0), "weathered_stone_normal.png")
    save(roughness_map(238, 12, 203), "weathered_stone_roughness.png")
...
def texture_roof_tile() -> None:
    """Ngói âm-dương đất nung cũ: nền cam gạch, sống ngói kem sáng, rãnh tối."""
    rng = random.Random(501)
    img = noise_base((174, 92, 55), 18, 502)
    draw = ImageDraw.Draw(img, "RGBA")
    height = Image.new("L", (SIZE, SIZE), 118)
    hd = ImageDraw.Draw(height)

    tile_w = 38
    # Pattern dọc để khi phủ lên các sống ngói, mái có cảm giác nhiều viên ngói như ảnh thật.
    for x in range(-tile_w, SIZE + tile_w, tile_w):
        warm = (196 + rng.randint(-18, 20), 115 + rng.randint(-12, 18), 70 + rng.randint(-10, 12), 132)
        pale = (232 + rng.randint(-10, 16), 202 + rng.randint(-16, 12), 165 + rng.randint(-18, 18), 132)
        draw.rounded_rectangle([x + 5, 0, x + 22, SIZE], radius=8, fill=pale)
        draw.line([(x + 2, 0), (x + 2, SIZE)], fill=(84, 48, 34, 118), width=2)
        draw.line([(x + 24, 0), (x + 24, SIZE)], fill=(97, 53, 36, 92), width=1)
        draw.rectangle([x + 24, 0, x + tile_w - 4, SIZE], fill=warm)
        hd.rounded_rectangle([x + 5, 0, x + 22, SIZE], radius=8, fill=186)
        hd.line([(x + 2, 0), (x + 2, SIZE)], fill=70, width=2)
        hd.line([(x + 24, 0), (x + 24, SIZE)], fill=82, width=1)

    # Ron ngang mảnh tạo từng hàng viên ngói, nhưng không làm mái thành sọc ngang quá rõ.
    for y in range(28, SIZE, 56):
        draw.line([(0, y), (SIZE, y + rng.randint(-2, 2))], fill=(78, 47, 35, 68), width=1)
        hd.line([(0, y), (SIZE, y)], fill=88, width=1)
        draw.line([(0, y + 2), (SIZE, y + 2)], fill=(245, 207, 165, 30), width=1)

    for _ in range(180):
        x, y = rng.randrange(SIZE), rng.randrange(SIZE)
        r = rng.randint(1, 4)
        color = rng.choice([(63, 48, 35, 55), (232, 185, 132, 45), (112, 61, 39, 55)])
        draw.ellipse([x - r, y - r, x + r, y + r], fill=color)

    save(img.filter(ImageFilter.GaussianBlur(0.10)), "old_roof_tile_basecolor.png")
    save(normal_from_height(height.filter(ImageFilter.GaussianBlur(0.45)), 4.4), "old_roof_tile_normal.png")
    save(roughness_map(232, 14, 503), "old_roof_tile_roughness.png")
