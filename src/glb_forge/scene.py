...
class SceneMesh:
    """Một mesh scene procedural Y-up có nhiều material.

    Writer xuất toàn bộ scene thành một mesh GLB, chia primitive theo material.
    Cách này giống barem ZIP mẫu: thuần Python, có UV, texture nhúng vào GLB.
    """
    ...
    def add_material(
        ...
        *,
        ...
        if roughness_texture is None and base_color_texture:
            base_path = Path(base_color_texture)
            candidate = base_path.with_name(base_path.name.replace("_basecolor", "_roughness"))
            if candidate.exists():
                roughness_texture = str(candidate)
    ...
    def push_vertex(self, position: Vec3, normal: Vec3, uv: Vec2 | None = None) -> int:
    ...
    def add_triangle(self, p0: Vec3, p1: Vec3, p2: Vec3, material: int, normal: Vec3 | None = None) -> None:
        ...
        i0 = self.push_vertex(p0, n)
        i1 = self.push_vertex(p1, n)
        i2 = self.push_vertex(p2, n)
    ...
    def add_quad(
        ...
        uv: tuple[Vec2, Vec2, Vec2, Vec2] | None = None,
    ) -> None:
        n = v_norm(v_cross(v_sub(p1, p0), v_sub(p2, p0))) if normal is None else v_norm(normal)
        base = len(self.positions)
        self.positions.extend([p0, p1, p2, p3])
        self.normals.extend([n, n, n, n])
        self.texcoords.extend(list(uv or ((0.0, 0.0), (1.0, 0.0), (1.0, 1.0), (0.0, 1.0))))
    ...
    def add_frustum(
        ...
        *,
        ...
        """Thêm hình trụ/nón cụt đứng theo trục Y."""
    ...
    def add_frustum_between(
        ...
        *,
        ...
        """Hình trụ/nón cụt theo đoạn thẳng bất kỳ."""
        axis = v_sub(p1, p0)
        length = v_len(axis)
        if length < 1e-6:
            return
    ...
    def add_lathe(
        ...
        *,
        ...
        """Tạo vật thể tròn xoay quanh trục Y từ profile (radius, y)."""
